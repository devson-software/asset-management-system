      ...
      <div class="col-12 flex justify-between items-center">
        <div>
          ...
          <div class="text-subtitle1 text-grey-7" v-if="activeProjectName">
            ...
            <span class="text-weight-bold text-primary">{{ activeProjectName }}</span>
          </div>
        ...
        <q-card flat bordered class="rounded-borders">
          ...
          >
              ...
              <q-td :props="props">
                <div class="row items-center no-wrap">
                  <q-avatar size="32px" class="q-mr-md bg-grey-1">
                    <q-icon
                      :name="
                        props.row.unitType?.includes('Cassette')
                          ? 'fas fa-square-poll-vertical'
                          : props.row.unitType?.includes('Split')
            ...
            <template v-slot:body-cell-status="props">
              <q-td :props="props">
                <q-badge
                  :color="props.row.status === 'Registered' ? 'positive' : 'orange'"
                  rounded
                  class="q-px-sm"
                >
                  {{ props.row.status }}
            ...
            <template v-slot:body-cell-actions="props">
              <q-td :props="props">
                <q-btn flat round color="grey-7" icon="fas fa-ellipsis-v" size="sm">
                  <q-menu
                    auto-close
                  ...
                  >
                    <q-list style="min-width: 180px">
                      ...
                      <q-item clickable :to="'/assets/' + props.row.id + '/history'">
                        <q-item-section avatar
                          ><q-icon name="fas fa-history" color="indigo" size="sm"
                        /></q-item-section>
...
export default defineComponent({
  ...
  setup() {
      ...
      store.customers.forEach((customer) => {
        customer.projects.forEach((project) => {
          ...
          project.assets.forEach((asset) => {
            const serviceCount = store.services.filter(
              (s) => s.unitRef === asset.unitRef && s.customer === customer.name,
            ).length
    ...
    const generateQRs = () => {
      $q.notify({
        message: 'Preparing high-resolution QR labels...',
        color: 'secondary',
        icon: 'qr_code',
      })
    }
    ...
    const exportToExcel = () => {
      $q.notify({
        message: 'Exporting assets register to Excel...',
        color: 'green-7',
        icon: 'file_download',
      })
    ...
    const printQR = (row) => {
      $q.notify({
        message: `Sent QR sticker for ${row.unitRef} to label printer.`,
        color: 'orange-8',
        icon: 'print',
      })
    }
